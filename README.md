...
```bash
Sqssample$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
Sqssample$ sam local start-api
Sqssample$ curl http://localhost:3000/
```
...
```bash
Sqssample$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
Sqssample$ cd HelloWorldFunction
